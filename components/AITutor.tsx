...
const STORAGE_KEY = 'elite_chat_vault_v2026';
...
export const AITutor = () => {
  const [sessions, setSessions] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : [{
      ...
      messages: [{ role: 'model', text: 'Welcome to Elite Academy. I am Mr. Elite, your AI Tutor. How can I assist your English journey today?' }]
    }];
  });
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white/5 rounded-[2.5rem] border border-white/10 overflow-hidden flex flex-col h-[650px] shadow-2xl backdrop-blur-3xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-right">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">System Active</span>
            </div>
          </div>
        </div>
        <button 
          onClick={() => {
            ...
            setSessions([{ id: newId, messages: [{ role: 'model', text: 'New session started. How can I help?' }] }, ...sessions]);
            setCurrentId(newId);
          }}
      ...
      <div ref={scrollRef} className="flex-grow p-8 overflow-y-auto space-y-8 bg-black/20 custom-scrollbar scroll-smooth">
        {currentSession.messages.map((m: any, i: number) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in`}>
            <div className={`max-w-[80%] p-5 rounded-3xl shadow-2xl text-sm leading-relaxed ${
              m.role === 'user' 
      ...
      <div className="p-8 bg-black/40 border-t border-white/5 flex gap-4">
        <input 
          ...
          placeholder="Type your message to Mr. Elite..." 
        />
        <button 
          onClick={handleSend}
          disabled={isLoading}
